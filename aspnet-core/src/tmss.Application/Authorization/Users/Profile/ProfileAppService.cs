...
using tmss.Authentication.TwoFactor.Google;
using tmss.Authorization.Users.Dto;
using tmss.Authorization.Users.Profile.Cache;
...
{
    [AbpAuthorize]
    public class ProfileAppService : tmssAppServiceBase, IProfileAppService
    {
        ...
        private readonly IRepository<MstSupplierContacts, long> _supplierContact;
        ...
        public ProfileAppService(
            ...
            IRepository<MstSupplierContacts, long> supplierContact
            )
        {
        ...
        public async Task<long?> GetCurrentUserSupplierId()
        {
            var user = await base.GetCurrentUserAsync();
            if (user.SupplierContactId.HasValue) {
                var contact = await _supplierContact.GetAll().AsNoTracking().FirstOrDefaultAsync(e => e.Id == user.SupplierContactId);
                if (contact != null) return contact?.SupplierId;
                else return 0;
            }
            return -1; //44;
        }
        ...
        public async Task<CurrentUserProfileEditDto> GetCurrentUserProfileForEdit()
        {
            ...
            userProfileEditDto.QrCodeSetupImageUrl = user.GoogleAuthenticatorKey != null
                ? _googleTwoFactorAuthenticateService.GenerateSetupCode("tmss",
