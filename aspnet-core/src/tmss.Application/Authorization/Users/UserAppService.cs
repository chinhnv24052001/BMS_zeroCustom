...
using tmss.Authorization.Permissions;
...
namespace tmss.Authorization.Users
{
    ...
    public class UserAppService : tmssAppServiceBase, IUserAppService
    {
        ...
        {
            return await _userRepo.FirstOrDefaultAsync(e => e.Id == userId);
        }
        ...
        private IQueryable<User> GetUsersFilteredQuery(IGetUsersInput input)
        {
            var query = UserManager.Users
                .WhereIf(!String.IsNullOrEmpty(input.Name), u=>u.Name.Contains(input.Name) || u.UserName.Contains(input.Name))
