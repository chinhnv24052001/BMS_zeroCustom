...
import { UiCustomizationSettingsDto, UserServiceProxy } from '@shared/service-proxies/service-proxies';
...
export abstract class AppComponentBase {
    ...
    //     this.userServiceProxy.getUserById(abp.session.userId)
    //     .pipe(finalize(()=>{
    ...
    checkValidateInputHasError(name: string){
        let err = document.getElementsByTagName(name)[0].getElementsByClassName("err-message");
        if (err.length == 0 ) return false;
        else {

            this.notify.warn("Vui lòng nhập đúng và đủ tất cả các thông tin yêu cầu");
            return true;
        }
    }
