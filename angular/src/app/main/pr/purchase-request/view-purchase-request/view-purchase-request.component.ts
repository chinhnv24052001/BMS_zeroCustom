...
@Component({
  ...
  styleUrls: ['./view-purchase-request.component.less']
})
export class ViewPurchaseRequestComponent extends AppComponentBase implements OnInit {
  ...
  listOrganizationAll: { label: string, value: string | number }[] = [];
  ...
  prDetail: GetPurchaseRequestForEditDto = new GetPurchaseRequestForEditDto();
  ...
  selectedAttachment: GetAllPurchaseRequestAttachmentsForViewDto = new GetAllPurchaseRequestAttachmentsForViewDto();
  ...
  listPurchaseRequestDetailApprove = [];
  listPurchaseRequestDetailReference = [];
  ...
  ngOnInit(): void {
    ...
    this.mstOrganizationsServiceProxy.getAllOrganizations().subscribe((res) => {
      res.forEach(e => {
        ...
        this.listOrganizationAll.push({ label: e.name, value: e.id })
    })
    });
  ...
  getRequesterInfo() {
    ...
    // this.commonGeneralCacheServiceProxy.geUserById(this.createOrEditForm.get('preparerId').value).subscribe((val) => {
    //   this.createOrEditForm.get('department').setValue(val.department);
    //   this.createOrEditForm.get('email').setValue(val.email);
    // });

    this.commonGeneralCacheServiceProxy.getRequesterInfo(this.createOrEditForm.get('preparerId').value)
      ...
      .subscribe(res => {
        ...
        this.createOrEditForm.get('email').setValue(res.email);
      });
  }
  ...
  // sitePopup() {
  //   this.listSite.show(this.formPRLines.get('suggestedVendorLocation').value, this.suppliers);
  // }

  // requesterPopup() {
  //   this.listRequester.show(this.formPRLines.get('requesterName').value, this.suppliers);
  // }


  // locationPopup() {
  //   this.listLocation.show(this.formPRLines.get('locationCode').value, this.suppliers);
  // }
  //#endregion popup
    ...
    if (this.agDataValidateService.checkDuplicateData(event, this.displayedData, fieldToCheck)) {
      ...
      this.commonGeneralCacheServiceProxy.getListOrgzationsByPartNo(event.partNo).subscribe((res) => {
        res.forEach(e => {
          ...
          this.gridParamsPrDetail.api.redrawRows();
          if (res.length > 0) {
            this.selectedNode.data.organizationId = res[0]?.id;
          }
          this.gridParamsPrDetail.api.applyTransaction({ update: [this.selectedNode.data] });
  ...
  buildFormPRHeaders() {
    this.createOrEditForm = this.formBuilder.group({
      ...
      preparerId: [this.appSession.user.id],
      division: [undefined],
      ...
      destinationOrganizationId: [undefined],
      deliverToLocationId: [undefined],
    });
  }

  // buildFormPRLines() {
  //   this.formPRLines = this.formBuilder.group({
  //     destinationTypeCode: [undefined, GlobalValidator.required],
  //     destinationOrganizationId: [0, GlobalValidator.required],
  //     requesterName: [undefined, GlobalValidator.required],
  //     toPersonId: [0],
  //     locationCode: [undefined],
  //     deliverToLocationId: [0],
  //     subinventory: [undefined],
  //     sourceTypeCode: ['Supplier', GlobalValidator.required],
  //     attribute11: [undefined],
  //     attribute10: [undefined],
  //     suggestedVendorName: [undefined],
  //     vendorId: [undefined],
  //     suggestedVendorLocation: [undefined, GlobalValidator.required],
  ...
  addRow() {
    ...
    if (this.selectedNode && this.selectedNode.data) {
      // Object.assign(this.selectedNode.data, this.formPRLines.getRawValue())
    }
  ...
  createPr() {
    ...
    // console.log(this.inputPurchaseRequestHeaderDto);
    this.spinnerService.show();
  ...
  callBackGridPrDetailAprrove(params: GridParams) {
    this.gridParamsPrDetailAprrove = params;
    params.api.setRowData(this.listPurchaseRequestDetailApprove);
  }

  callBackGridPrDetaillReference(params: GridParams) {
    this.gridParamsPrDetailReference = params;
    params.api.setRowData(this.listPurchaseRequestDetailReference);
    params.api.sizeColumnsToFit();
  }

  replyRequest(){
    this._requetApproval.replyFromHeader(this.currentIdPr,this.prDetail.replyNote).subscribe(e => {
        this.notify.success("Successfully Reply")
    })
  }

  show(id: number, isDisabled: boolean) {
    this.currentIdPr = id;
    ...
    this.purchaseRequestServiceProxy.getPurchaseRequestById(id).subscribe((result) => {
      ...
      this.prDetail = result;
      let name = "";
      if((this.prDetail.replyNote ?? "").split(":").length >1) name = (this.prDetail.replyNote ?? "").split(":")[0] ;
      this.prDetail.replyNote = (this.prDetail.replyNote ?? "").slice(name.length >= 1 ? name.length + 1 : 0,(this.prDetail.replyNote ?? "").length);
  ...
  getApprovalInfos(id: number) {
    this.spinnerService.show();
    this._serviceProxy.getAllApprovalInfo(id,"PR")
      .pipe(finalize(() => this.spinnerService.hide()))
      .subscribe(res => {
  ...
  getReference(id: number) {
    this.spinnerService.show();
    this.commonGeneralCacheServiceProxy.getListReference(id, 'PR')
      .pipe(finalize(() => this.spinnerService.hide()))
      .subscribe(res => {
  ...
  onUpload(files: Array<any>): void {
    if (files.length > 0) {
      ...
      // this.listFileAttachment = this.uploadData;
  ...
  onChangeAttachmentSelection(params: any) {
    ...
    this.selectedAttachment = params.api?.getSelectedRows()[0] ?? new GetAllPurchaseRequestAttachmentsForViewDto();
  }
  ...
  dowloadAttachment(isCell: boolean, gridParams: any) {
    if (isCell) {
      ...
      this.prDetail.attachments.forEach(file => {
        this._http
  ...
  //   this.prDetail.attachments = [];
  //   this.prDetail.attachments = this.gridTableService.getAllData(this.selectedAttachmentNode);
  // }
    ...
    if (this.selectedAttachmentNode.data?.id) {
      this.message.confirm(this.l('AreYouSure'), this.l('DeleteThisSelection'), (isConfirmed) => {
        if (isConfirmed) {
          ...
          this.uploadData  = this.gridTableService.getAllData(this.selectedAttachmentNode);
        }
      })
    }
    else {
      this.attachmentsParams?.api.applyTransaction({ remove: [this.selectedAttachmentNode.data] });
      // this.selectedRowPrDetail.attachments = [];
      this.uploadData = this.gridTableService.getAllData(this.attachmentsParams);
    }
  }
