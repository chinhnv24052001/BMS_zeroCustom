...
import { BmsBudgetAdditionComponent } from './bms-budget-addition/bms-budget-addition.component';

@Component({
  selector: 'app-bms-budget-transfer-user',
  templateUrl: './bms-budget-transfer-user.component.html',
  styleUrls: ['./bms-budget-transfer-user.component.css']
})
export class BmsBudgetTransferComponent extends AppComponentBase implements OnInit {
  ...
  @ViewChild(' bmsBudgetAddition', { static: true }) bmsBudgetAddition: BmsBudgetAdditionComponent;
  gridForm: FormGroup;
  ...
  onChangeSelection(params) {
    ...
    this.isGroupReceived(this.selectedRow.id);
  }
  ...
  submit() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.createOrEditBmsBudgetTransferUser.show(this.selectedRow.id, this.selectedRow.typeId, 2, 1); //(id row,  type transfer, form?, rejeject or approval)
    } else {
      this.notify.warn(this.l(AppConsts.CPS_KEYS.Please_Select_1_Line_To_Edit));
    }
  }
  ...
  reject() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.createOrEditBmsBudgetTransferUser.show(this.selectedRow.id, this.selectedRow.typeId, 2, 2);
    } else {
      this.notify.warn(this.l(AppConsts.CPS_KEYS.Please_Select_1_Line_To_Edit));
    }
  }
  ...
  isMAFinUser() {
    this._mainComponentServiceProxy.isMAFinUser().subscribe(result => {
      this.isMAFin = result;
    })
  }
  ...
  isGroupReceived(id) {
    setTimeout(() => {
      this._mainComponentServiceProxy.isGroupReceived(id).subscribe(result => {
        this.isGroupRece = result;
        //disable group approval
        if ((!this.isGroupRece && this.selectedRow.statusId == 2)
          || (this.isGroupRece && (this.selectedRow.statusId == 4 || this.selectedRow.statusId == 2) )) {
          this.disableGroupApproval = false;
          this.disableGroupReject = false;
        }
        else {
          this.disableGroupApproval = true;
          this.disableGroupReject = true;
        }
      })
  }, 1000);
  }
