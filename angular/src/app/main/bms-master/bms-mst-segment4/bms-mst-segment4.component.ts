...
import { BmsMstPeriodServiceProxy, BmsMstSegment1ServiceProxy, BmsMstSegment4ServiceProxy, MstPeriodServiceProxy, MstSegment1Dto, MstSegment4Dto } from '@shared/service-proxies/service-proxies';
import { ceil } from 'lodash-es';
import { finalize } from 'rxjs/operators';
import { CreateOrEditBmsMstSegment4Component } from './create-or-edit-bms-mst-segment4/create-or-edit-bms-mst-segment4.component';
import { BmsImportSegmentComponent } from '../bms-import-segment/import-segment.component';

@Component({
  selector: 'app-bms-mst-segment4',
  templateUrl: './bms-mst-segment4.component.html',
  styleUrls: ['./bms-mst-segment4.component.css']
})
export class BmsMstSegment4Component extends AppComponentBase implements OnInit {
  @ViewChild(' createOrEditBmsMstSegment4', { static: true })  createOrEditBmsMstSegment4: CreateOrEditBmsMstSegment4Component;
  ...
  listSegment4: MstSegment4Dto[];
  selectedRow: MstSegment4Dto = new MstSegment4Dto();
  ...
  ngOnInit(): void {
    ...
    this.gridColDef = [
      ...
      {
        headerName: this.l('GroupSeg4'),
        headerTooltip: this.l('GroupSeg4'),
        field: 'groupSeg4Name',
        maxWidth: 200
      },
  ...
  onChangeSelection(params) {
    this.selectedRow =
      params.api.getSelectedRows()[0] ?? new MstSegment4Dto();
    this.selectedRow = Object.assign({}, this.selectedRow);
  }

  changePaginationParams(paginationParams: PaginationParamsModel) {
    if (!this.listSegment4) {
      return;
    }
  ...
  search() {
    ...
    this._bmsMstSegment4ServiceProxy.getAllSegment4(
      ...
      .subscribe((result) => {
        this.listSegment4 = result.items;
        this.gridParams.api.setRowData(this.listSegment4);
  ...
  add() {
    this.createOrEditBmsMstSegment4.show();
  }

  edit() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.createOrEditBmsMstSegment4.show(this.selectedRow.id);
    } else {
  ...
  delete() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.message.confirm('', this.l(AppConsts.CPS_KEYS.Are_You_Sure), (isConfirmed) => {
        if (isConfirmed) {
          this.spinnerService.show();
          this._bmsMstSegment4ServiceProxy.delete(this.selectedRow.id).subscribe(val => {
