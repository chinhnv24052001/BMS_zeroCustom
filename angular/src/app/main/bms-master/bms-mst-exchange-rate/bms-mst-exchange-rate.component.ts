...
import { BmsMstPeriodServiceProxy, BmsMstSegment1ServiceProxy, BmsMstSegment4GroupServiceProxy, ExchangeRateMasterServiceProxy, MstCurrencyServiceProxy, MstExchangeRateDto, MstPeriodServiceProxy, MstSegment1Dto, MstSegment4GroupDto } from '@shared/service-proxies/service-proxies';
import { ceil } from 'lodash-es';
import { finalize } from 'rxjs/operators';
import { BmsImportSegmentComponent } from '../bms-import-segment/import-segment.component';
import { CreateOrEditBmsMstExchangeRateComponent } from './create-or-edit-bms-mst-exchange-rate/create-or-edit-bms-mst-exchange-rate.component';

@Component({
  selector: 'app-bms-mst-exchange-rate',
  templateUrl: './bms-mst-exchange-rate.component.html',
  styleUrls: ['./bms-mst-exchange-rate.component.css']
})
export class BmsMstExchangeRateComponent extends AppComponentBase implements OnInit {
  @ViewChild(' createOrEditBmsMstExchangeRate', { static: true })  createOrEditBmsMstExchangeRate: CreateOrEditBmsMstExchangeRateComponent;
  @ViewChild('bmsImportSegment', { static: true }) bmsImportSegment: BmsImportSegmentComponent; 
  gridForm: FormGroup;
  gridColDef: CustomColDef[];
  paginationParams: PaginationParamsModel = { pageNum: 1, pageSize: 20, totalCount: 0, totalPage: 0, sorting: '', skipCount: 0 };
  gridParams: GridParams | undefined;
  listGrid: MstExchangeRateDto[];
  selectedRow: MstExchangeRateDto = new MstExchangeRateDto();
  ...
  constructor(
    injector: Injector,
    private _mainComponentServiceProxy: ExchangeRateMasterServiceProxy,
  ...
  ngOnInit(): void {
    ...
    this.gridColDef = [
      ...
      },
      {
        ...
        headerTooltip: this.l('ExchangeRate'),
        field: 'exchangeRate', 
        minWidth: 200
      },
      {
...
selectDropDownCurrency() {
  ...
  onChangeSelection(params) {
    this.selectedRow =
      params.api.getSelectedRows()[0] ?? new MstExchangeRateDto();
  ...
  search() {
    ...
    this._mainComponentServiceProxy.getAllExchangeRate(
      this.gridForm.get('currencyId').value,
  ...
  add() {
    this.createOrEditBmsMstExchangeRate.show();
  }

  edit() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.createOrEditBmsMstExchangeRate.show(this.selectedRow.id);
    } else {
