...
import { BmsMstPeriodServiceProxy, BmsMstSegment1ServiceProxy, BmsMstSegment5ServiceProxy, MstPeriodServiceProxy, MstSegment1Dto, MstSegment5Dto } from '@shared/service-proxies/service-proxies';
import { ceil } from 'lodash-es';
import { finalize } from 'rxjs/operators';
import { CreateOrEditBmsMstSegment5Component } from './create-or-edit-bms-mst-segment5/create-or-edit-bms-mst-segment5.component';
import { BmsImportSegmentComponent } from '../bms-import-segment/import-segment.component';

@Component({
  selector: 'app-bms-mst-segment5',
  templateUrl: './bms-mst-segment5.component.html',
  styleUrls: ['./bms-mst-segment5.component.css']
})
export class BmsMstSegment5Component extends AppComponentBase implements OnInit {
  @ViewChild(' createOrEditBmsMstSegment5', { static: true })  createOrEditBmsMstSegment5: CreateOrEditBmsMstSegment5Component;
  ...
  listSegment5: MstSegment5Dto[];
  selectedRow: MstSegment5Dto = new MstSegment5Dto();
  ...
  search() {
    ...
    this._bmsMstSegment5ServiceProxy.getAllSegment5(
      ...
      .subscribe((result) => {
        this.listSegment5 = result.items;
        this.gridParams.api.setRowData(this.listSegment5);
  ...
  add() {
    this.createOrEditBmsMstSegment5.show();
  }

  edit() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.createOrEditBmsMstSegment5.show(this.selectedRow.id);
    } else {
  ...
  delete() {
    if (this.selectedRow.id && this.selectedRow.id > 0) {
      this.message.confirm('', this.l(AppConsts.CPS_KEYS.Are_You_Sure), (isConfirmed) => {
        if (isConfirmed) {
          this.spinnerService.show();
          this._bmsMstSegment5ServiceProxy.delete(this.selectedRow.id).subscribe(val => {
