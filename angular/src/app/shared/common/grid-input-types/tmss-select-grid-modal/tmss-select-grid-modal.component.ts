...
export class TmssSelectGridModalComponent extends AppComponentBase
    ...
    @Input() isPatchDataList = false;
    @Input() isExport: boolean = false;
    @Input() urlExport!: string;
    ...
    async search(value?: any) {
        ...
        if (this.apiCall && !this.isHideSearchField) {
            const api = this.showPagination == false
            ...
            this.list = this.listInput;
            this.paginationParams.pageSize = this.list.length;
            this.paginationParams.totalCount = this.list.length;
        }
    ...
    async show(val?: any | undefined, list?: any[] | undefined, total?: any | number | null | undefined, filterName?: any | null | undefined) {
        ...
        if (list) {
            ...
            this.list = this.filterName !== null ? list.filter(e => e[this.filterName.toString()] ? e[this.filterName.toString()].toString().includes(val ?? '') : ''.includes(val)) : list;
        }
        if (total) {
    ...
    export() {
        if (this.gridApi.getSelectedRows().length == 0) {
            this.notify.error('Please select some rows to export');
            return;
        }
        else {
            this.spinnerService.show();
            this.paramExport = this.gridApi.getSelectedRows();
            this._http.post(`${this.urlBase}/api/MasterExcelExport/${this.urlExport}`, this.paramExport,
                { responseType: 'blob' }).pipe(finalize(() => {
                    this.spinnerService.hide();
                })).subscribe(blob => {
                    FileSaver.saveAs(blob, this.fileNameExport);
                });
        }
