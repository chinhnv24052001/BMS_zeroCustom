...
import { AgCellEditorParams, AgCellPositionParams, CustomColDef, FrameworkComponent, GridParams } from '@app/shared/models/base.model';
...
export class GridTableComponent extends AppComponentBase implements OnInit {
  ...
  @Input() textPagination: string = this.l('InTotal');
...
onRowDragStart(event: AgCellEditorParams) {
    return this.rowDragEnter.emit(event);

}

onRowDragMove(event: AgCellEditorParams) {
...
navigateToNextCell(params: { event: KeyboardEvent,keyCode: number, key: string, nextCellPosition: AgCellPositionParams, previousCellPosition: AgCellPositionParams }) {
  ...
  switch (params.event.keyCode) {
      case KEY_UP:
          ...
          if (!this.isSuppressRowClickSelection){
            ...
            this.params.api.getDisplayedRowAtIndex(nextRowIndex)?.setSelected(true);
            // this.params.api.getDisplayedRowAtIndex(nextRowIndex-1)?.setSelected(false);
          }
          return {
