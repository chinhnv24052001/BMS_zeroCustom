...
export class DataFormatService {
    ...
    moneyFormat(value: Number | number | string) {
      return value ? Math.round(Number(value)).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : '';
    }

    floatMoneyFormat(value: any) {
      return value ? Number(value).toFixed(2).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : '';
    }

    // floatMoneyFormat(val: number | string) {
    //   if (val) {
    ...
    dateFormat(val: string | moment.Moment | Date) {
      ...
      return val ? moment(val.toString()).tz(this.getTimeZone())?.format('DD/MM/YYYY') : '';
    }
    ...
    dateTimeFormat(val: any) {
      // if (val == '0001-01-01T00:00:00' || val == '9999-12-31T23:59:59.9999999') return '';
      return val ? moment(val.toString()).tz(this.getTimeZone())?.format('DD/MM/YYYY HH:mm') : '';
    }
